# https://github.com/Crivella/ocr_translate/blob/master/run_server.py
# https://github.com/boysugi20/python-image-translator/blob/master/Pipfile
...
parser.add_argument('--from', dest='source_lang', help='Source language code', required=True)
parser.add_argument('--to', dest='target_lang', help='Target language code', required=True)
...
reader = easyocr.Reader([args.source_lang, "en"], model_storage_directory = 'model')

# Initialize the Translator
translator = GoogleTranslator(source=args.source_lang, target=args.target_lang)

# Process each image file from arguments
for image_path in args.images:
    ...
    # Save modified image
    base_filename, extension = os.path.splitext(image_path)
    output_path = f"{base_filename}.{args.target_lang}{extension}"
